...
class Category(BaseModel):
    name = models.CharField(
        max_length=MAX_LENGTH_OF_CATEGORY_NAME, verbose_name="Название"
    )
...
class Post(BaseModel):
    ...
    subtitle = models.CharField(
        max_length=MAX_LENGTH_OF_SUBTITLE, verbose_name="Подзаголовок"
    )
    text = models.TextField(verbose_name="Текст статьи")
    slug = models.SlugField(unique=True, verbose_name="Идентификатор", blank=True)
    pub_date = models.DateTimeField(
        verbose_name="Дата и время публикации", default=timezone.now
    )
