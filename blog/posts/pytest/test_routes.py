...
def test_public_routes(anonymous_client, url, request):
...
def test_availability_for_post_and_category_adding(
    client, url, request, data_fixture, expected_status
):
    url = request.getfixturevalue(url)
...
def test_availability_for_subscribers_list_and_detail(
    client, url, request, expected_status
):
    url = request.getfixturevalue(url)
    client = request.getfixturevalue(client)
