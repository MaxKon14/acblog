...
def test_auto_post_slug_generation(author_client, posts_list_url, post_data):
...
def test_auto_category_slug_generation(
    author_client, categories_list_url, category_data
):
...
def test_creating_post_with_existing_slug(
    author_client, posts_list_url, post, category
):
    posts_before = Post.objects.count()
    data = {
        "title": post.title,
...
def test_subscriber_email_must_be_unique(
    ...
    response = anonymous_client.post(
        subscribers_list_url, data=email_data, format="json"
    )
