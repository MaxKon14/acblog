...
def test_only_posts_with_published_categories_in_list(
        posts_list_url, post_detail_url, post, anonymous_client, author_client, category, category_detail_url,
        categories_list_url, unpublished_category_data
):
    response = anonymous_client.get(posts_list_url)
    response_json = response.json()
    results = response_json['results']
    ...
    new_category_response = author_client.post(
        categories_list_url,
        data = unpublished_category_data,
        format='json'
    )
